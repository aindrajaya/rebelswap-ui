...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  apiRouter.post("/trades/calculate", async (req, res) => {
    ...
    if (USE_MOCK_DATA) {
      ...
      const { strategy_id, total_usd_size, user_address } = req.body as { strategy_id: keyof typeof strategies; total_usd_size: number; user_address?: string };
      ...
      console.log(`Calculating for ${assetCount} assets with ${sizePerAsset} USD per asset`);
      ...
      const response = {
        ...
        strategy_id,
        trade_parameters,
        user_address_context: user_address || "0xe45ee40d56ec816290886e8c91378c50e8f16ec9"
      };
  ...
  apiRouter.post("/trades/execute", async (req, res) => {
    try {
      ...
      if (!strategy_id || !trade_parameters || !signature || !user_address) {
        return res.status(400).json({
          error: "Missing required parameters",
          details: "strategy_id, trade_parameters, signature, and user_address are required"
        });
      }
